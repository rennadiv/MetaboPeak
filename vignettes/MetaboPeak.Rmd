...
This vignette demonstrates a complete metabolomics workflow using the example datasets included in the package. The data originate from a metabolomics experiment investigating the effects of elevated CO2,  and drought on two tree species.
...
### Data structure
Most of the functions require mass spectrometry data as shown in example. First column is the peak identification code, than there are the samples, mass to charge ration and retention time. In rows are the peaks with abundances for the samples and their measured m/z and RT.
...
The example dataset included with the package has already been processed using `treatGroup()`, which makes and unique code for combination of treatments. When analyzing your own experiments, apply `treatGroup()` to your treatment table before using downstream functions.
